...
const nextSkill = async () => {
  const value = parseInt(
    ((skill1.value + skill2.value + skill3.value) / 3).toString()
  );
...
<template>
  <div class="evaluation">
    <div class="label izq">
      <!-- label de puntuacion alineado ala izquierda -->
      Puntuacion del 1 a 5
    </div>
    <div class="level izq">
      <div class="list">
        <!-- label de las puntuaciones   -->
    ...
    <div class="SeccionPreguntas">
      ...
      <div class="cuestionario">
        <div class="preguntas">
          ...
          <p>{{ softskills[currentSkill][2] }}</p>
          <!-- preguntas que se mostraran -->
          <p>{{ softskills[currentSkill][3] }}</p>
        </div>
        <div class="puntuacion">
          <div class="respuesta">
            <!-- radio buttons para responder las preguntas -->
            <div>
              <input
                type="radio"
                v-model="skill1"
          ...
          <!-- fin de radio button para responder las preguntas -->
        </div>
      </div>
    </div>

    <div id="continuar">
      <button v-if="!terminated" class="btn_continuar" @click="nextSkill">
        Continuar
      </button>
      <!-- accion del boton continuar -->
      <router-link v-else class="btn_continuar" to="/">Terminar</router-link>
      <!-- accion del boton continuar -->
    </div>
  </div>
</template>
